...
Provider credentials are injected at container start by `scripts/docker-entrypoint.sh` (which writes `/home/app/.local/share/opencode/auth.json` before `opencode serve`): `ZAI_CODING_API_KEY` (alias `ZAI_API_KEY`, for `zai-coding-plan/` models such as `glm-5.3-flash`), `OPENROUTER_API_KEY`, and `MODEL_STUDIO_API_KEY` (Alibaba Model Studio Singapore, provider `bailian-payg`; sets provider auth only — the default model comes from `image/.opencode/opencode.json`, currently `qwencloud/qwen3.7-max` for the orchestrator — authenticated by the `QWENCLOUD_TOKEN_PLAN_API_KEY` compose env passthrough, not `auth.json` — and `zai-coding-plan/glm-5.3-flash` for subagents and the small model). Compose reads these from the host/CI environment — **no project `.env` file**.
...
| `QWENCLOUD_TOKEN_PLAN_API_KEY` | *(required)* | QwenCloud Token Plan key for the default model `qwencloud/qwen3.7-max` |
...
| `OPENCODE_MODEL` | `qwencloud/qwen3.7-max` | Model used for dispatched runs |
