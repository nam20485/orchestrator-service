...
graph TB
    ...
    subgraph Agent["orchestratorservice (OpenCode server, :4099)"]
        OC[Agent session<br/>permission: allow + external_directory deny]
    end
...
- **Permission model.** The orchestrator's opencode configuration sets a fail-closed permission block server-side (`"*": "allow"` plus `external_directory: {"*": "deny"}`) and dispatches are launched WITHOUT `--auto`: tool calls inside the session working directory run without approval (headless dispatch has no human to answer a prompt), while any path outside the working directory fails fast with deny — enforced for every session including task-spawned subagents. A permission `ask` can then only occur if the config fails to load, in which case the watchdog kills the run (fail-closed). The agent's effective capability boundary is the session working directory plus its configured tools/MCP servers, not a per-call approval gate.
