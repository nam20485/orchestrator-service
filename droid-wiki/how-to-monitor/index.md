...
3. **Permission-ask deadlock** (`permission_ask_grace_secs`, default `60`) — an unanswered `message=asking ... permission=<type>` line in the *server's* log. In a headless dispatch (no `--auto`; permission policy is server-side fail-closed) this can never be answered, so it fails fast instead of waiting out the idle timeout.
