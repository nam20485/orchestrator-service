...
- **Subagent `external_directory` permission lives in each agent's frontmatter** (`image/.opencode/agents/*.md` → `permission:`), NOT in the global `image/.opencode/opencode.json` `permission.external_directory` block — that global block is bypassed by `--dangerously-skip-permissions` and never reaches subagents. Mechanism: a subagent evaluates `Permission.merge(agent.permission, session.permission)` (`packages/opencode/src/session/tools.ts`); `session.permission` is derived via `deriveSubagentSessionPermission` from the PARENT session, which is empty because the orchestrator runs skip-permissions — so only the subagent's OWN frontmatter rules apply. An agent with no `external_directory` rule defaults to `ask`, which is unanswerable in headless fire-and-forget runs → the run hangs until the idle watchdog kills it (`IDLE_TIMEOUT_SECS`, 1800s). Every specialist agent therefore declares `external_directory: { "*": "deny", "/tmp/**": "allow" }` (fail-fast: `/tmp` scratch — body files, driver scripts — is allowed; any other external path denies immediately via `BlockedError` instead of hanging). Precedence: opencode v1.18.4 `evaluate()` uses `findLast` (last-match-wins), so the specific `/tmp/**` allow (listed last) overrides the catch-all `*: deny` (first). Also note `Wildcard.match` maps `*`→`.*` and crosses `/` (no `**` distinction, so `/tmp/*` == `/tmp/**`). Commits: `2496bb3` (scoped `/tmp/**` allow + opencode.json comment + removed `agent-instructions-expert`), `ed7c053` (fail-fast deny-default).
