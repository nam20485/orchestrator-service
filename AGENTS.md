...
Notes: `prompt.ps1`/`attach.ps1` resolve server URL as: `-ServerUrl` > `$OPENCODE_SERVER_URL` > `$OPENCODE_HOST`/`$OPENCODE_PORT` > `http://localhost:4099`. They rely on host `OPENCODE_SERVER_PASSWORD` (never hardcoded). Default model in `prompt.ps1` is `zai-coding-plan/glm-5`. `prompt.ps1` passes `--auto` by default (param `$Auto = "true"`) and appends `--variant <value>` only when `-Variant` is supplied (default empty → omitted); it also supports `-Thinking`/`-PrintLogs`.
