...
- **Subagent scratch must be written INSIDE the workspace, never `/tmp`** (HEADLESS DEADLOCK RULE). These dispatches run `opencode run --attach ... --dangerously-skip-permissions` with no human to answer prompts. opencode v1.18.4 has a subagent permission-inheritance bug (issue #30527 cluster, with #12566/#20864/#27497/#28052): a task-spawned subagent does NOT receive the parent session's (skip-permissions → empty) `external_directory` rules, NOR reliably its own frontmatter `external_directory` allow — `session/prompt.ts` clobbers `session.permission` (replace, not merge) with only per-call tool denies. So any write OUTSIDE the project `--dir` (`/workspace/<slug>`) resolves to `external_directory → ask`, which is unanswerable headless → the subagent blocks forever and the run hangs until the watchdog kills it. The prior `/tmp/**` allow fixes (`opencode.json` `permission.external_directory` + per-agent `image/.opencode/agents/*.md` frontmatter) are **confirmed deployed but INERT** for task subagents in this version — verified by the golf38 run (image built with both, subagent still got `ask` for `/tmp/kilo/<repo>/*`). Fix: write scratch to `<workspace>/.scratch/` (in-project paths never trigger `external_directory`). The watchdog now also fails-fast on any unanswered `message=asking` server-log line after `PERMISSION_ASK_GRACE_SECS` (60s). The agent frontmatter `external_directory: {"*": "deny"}` is retained only as defense-in-depth (fail-fast IF honored); do NOT rely on it. `Wildcard.match` maps `*`→`.*` across `/` (no `**` distinction).
