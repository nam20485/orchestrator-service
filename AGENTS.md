...
- Non-root execution: all three containers run as non-root (`app` UID 1000 by default via gosu entrypoint; Caddy uses upstream `caddy` user). Workspace files are operator-owned — no `sudo` for cleanup. Override UID/GID via `APP_UID`/`APP_GID` env vars. One-time migration for pre-existing root-owned files: `sudo chown -R $(id -u):$(id -g) $WORKSPACE_DIR`.
