...
- **Permission model: fail-closed deny-default in `opencode.json`, NO `--auto`** (2026-09-10, closes PR #49 review HIGH finding; see `plan_docs/opencode-1.18.30-gap-analysis.md`). `image/.opencode/opencode.json` sets `"permission": {"*": "allow", "external_directory": {"*": "deny"}}` — everything is allowed (no `ask` can fire, so headless runs cannot deadlock) EXCEPT paths outside the session working directory, which fail fast with deny. Verified empirically on opencode 1.18.30 (and 1.18.4): the block governs the orchestrator session AND task-spawned subagents, with or without agent frontmatter; MCP tools and task delegation unaffected. `scripts/prompt.ps1` deliberately does NOT pass `--auto`: if the config ever fails to load, `external_directory` falls back to its `ask` default, the run deadlocks, and the watchdog `_PermissionAskMonitor` (60s grace, `permission_deadlock` kill) terminates it — fail-closed instead of silently auto-approving. History: the earlier `"permission": "allow"` + `--auto` model (2026-07-24) replaced a phantom `--dangerously-skip-permissions` flag (rejected opencode PR #14583) but was fail-open; the golf38-era `external_directory` allows/denies were believed inert for subagents, which 2026-09-10 probes could not reproduce (see gap analysis §4). Subagent scratch stays in-workspace (`<workspace>/.scratch/`) as defense-in-depth. **Known residual risk (flagged in PR #49 review):** the `external_directory` deny gates path-bearing file tools (read/edit/write/glob/grep) but NOT `bash` command operands, and the specialist agents that need bash (developer, github-expert, qa-test-engineer) run with it allowed — prompt-injected bash could read sibling `/workspace/` trees or container secrets (`auth.json`, `GH_ORCHESTRATION_AGENT_TOKEN`). Bounded today by the fail-closed `DIRECT_BODY_ALLOWED_SENDERS` gate and workflow-structured (non-arbitrary) prompts on other labels; a bash command allowlist is the recorded follow-up (deferred as deadlock-prone — see the gap analysis).
...
- **Single-writer invariant:** only the Orchestrator may call the write tools (`create_entities`/`create_relations`/`add_observations`/`delete_*`). Subagents and specialists are READ-ONLY (`search_nodes`/`open_nodes`/`read_graph`); they return durable facts via a `## Memory Save Requests` list that the Orchestrator persists. Concurrent writers (one server-memory process per OpenCode session) corrupt `memory.jsonl`.
