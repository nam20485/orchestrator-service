...
| Category | Examples |
|----------|----------|
| API keys | `AKIA…`, `sk-or-v1-…`, `sk-…` |
| Tokens | `ghp_…`, `ghs_…`, `github_pat_…`, `xox…`, `Bearer …` |
| Passwords | `--password "…"`, `password = "…"` |
| Secrets | Private key blocks, `api_key = "…"`, auth.json `"key": "…"` entries |
| PII | SSN patterns, email addresses, phone numbers |

Allowlisted placeholders (for example `FAKE-KEY-FOR-TESTING`, `example.com`) are skipped.
...
- Provider credentials belong in host or CI environment variables (`ZAI_CODING_API_KEY`, etc.), not committed files
