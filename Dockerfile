...
# Trust all bind-mounted repos under /workspace for root so git does not refuse
# operations with "fatal: detected dubious ownership" (CVE-2022-24765). The
# container only ever operates on the /workspace bind mount, whose per-session
# project dirs are created at runtime by the host (UID 1000) — paths git can't
# know at build time. safe.directory is NOT recursive, so enumerating only
# /workspace wouldn't cover /workspace/<project>/; '*' is the git-documented
# pattern for root-running containers on bind-mounted repos of arbitrary owner.
