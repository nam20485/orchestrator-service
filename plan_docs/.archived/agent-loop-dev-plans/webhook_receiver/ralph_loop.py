...
def check_bead_status(repo_path: str, bead_id: str) -> str:
    ...
    try:
        result = subprocess.run(
            ["br", "status", bead_id], cwd=repo_path, capture_output=True, text=True, check=True
        )
...
def spawn_agent(
    repo_path: str, bead: dict, retry_count: int, previous_logs: str = ""
) -> tuple[bool, str]:
...
def run_loop(repo_path: str):
    ...
    while True:
        ...
        if current_retries >= MAX_RETRIES:
            logger.error(
                f"🚨 Bead {bead_id} exceeded max retries. Halting loop to await human intervention."
            )
            break  # Stop loop if we hit a hard failure wall

        # Execute task
        success, logs = spawn_agent(
            repo_path, next_bead, current_retries, retry_state[bead_id]["logs"]
        )
